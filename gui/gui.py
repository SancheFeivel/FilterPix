...
class MainApp:
    ...
    def _init_state(self):
        ...
        self.animation_base_text = "Processing"
    ...
    def animate_processing_dots(self):
        ...
        dots = "." * self.dot_count
        progress_text = f"{self.animation_base_text}{dots}"  
    ...
    def setup_homescreen(self):
        ...
        self.box_2 = self.canvas.create_image(420.0, 245.0, image=self.images.load("box_2.png"), state='hidden')
    ...
    def set_blur_level(self, level):
        ...
        self.high_button.config(image=self.images.get("high_on.png" if level == 20 else "high_off.png"))
    ...
    def default_clicked(self):
        self.output_directory = self.folder_path  # Could be None!
    ...
    def folder_clicked(self):
        """Opens the selected folder or output folder if processing is complete."""
        print("DEBUG: folder_clicked called")
        try:
            # Determine which folder to open
            folder_to_open = None
            
            # If we have output stats and a custom output directory was used, open that
            if (self.sorter_stats or self.detection_stats) and self.output_directory and self.output_directory != self.folder_path:
                folder_to_open = self.output_directory
                print(f"Opening custom output folder: {folder_to_open}")
            # Otherwise open the input folder
            elif self.folder_path:
                folder_to_open = self.folder_path
                print(f"Opening input folder: {folder_to_open}")
            ...
            if folder_to_open and os.path.exists(folder_to_open):
                ...
                print(f"Folder does not exist: {folder_to_open}")
        except Exception as e:
    ...
    def start_clicked(self):
        ...
        self.canvas.itemconfig(self.box_2, state='normal')
        self.canvas.itemconfig(self.label_1, text="Processing")
    ...
    def _show_error(self, message):
        ...
        self.canvas.lift(self.box_2)
        self.canvas.lift(self.label_1)
        self._update_status(message)
        
    def _hide_error(self):
        """Hide error message and box_2"""
        if hasattr(self, 'box_2'):
            self.canvas.itemconfig(self.box_2, state='hidden')
        
        self.canvas.itemconfig(self.label_1, text="")
        self._update_status("")
    ...
    def detection_progress_callback(self, current, total, stage_name="processing"):
        stage_messages = {
            "star_check": "Checking Ratings",
            "sharpness_check": "Analyzing Sharpness",
            "sharpness_results": "Processing Results",
            "copying_sharp": "Copying Images",
            "copying_rejected": "Archiving Rejects",
            "sorting": "Sorting images"
        }
        
        stage_text = stage_messages.get(stage_name, stage_name.replace("_", " ").title())
        
        if current == -1:
            # Animated dots mode - set base text and ensure animation is running
            def update_with_animation():
                if self.is_processing:
                    self.animation_base_text = stage_text  # SET BASE TEXT
                    if not self.animation_id:  # Only start if not already running
                        self.start_animation()
            self.root.after(0, update_with_animation)
        else:
            # Progress counter mode - stop animation and show numbers
            progress_text = f"{stage_text}: {current}/{total}"
            def update_if_processing():
                if self.is_processing:
                    self.stop_animation()
                    self._update_status(progress_text)
            self.root.after(0, update_if_processing)
    ...
    def get_final_stats(self):
        ...
        if self.solo_detection:
            # Try each key in order, use first non-None value
            total_images = 0
            for key in ["total_images", "images_processed", "total"]:
            ...
            selected_images = 0
            for key in ["selected_images", "rated_images", "final_selection"]:
                val = detection.get(key)
                if val is not None:
                    selected_images = val
                    break
            
            detection_time = detection.get("elapsed_time") or 0
            detection_time = float(detection_time) if detection_time else 0.0
        ...
        total_images = 0
        for key in ["total_images", "images_processed"]:
            val = sorter.get(key)
            if val is not None:
                total_images = val
                break
        ...
        selected_images = 0
        for key in ["final_selection", "sharp_images"]:
            val = sorter.get(key)
            if val is not None:
                selected_images = val
                break

        sorter_time = sorter.get("elapsed_time") or 0
        detection_time = detection.get("elapsed_time") or 0
        
        sorter_time = float(sorter_time) if sorter_time else 0.0
        detection_time = float(detection_time) if detection_time else 0.0
